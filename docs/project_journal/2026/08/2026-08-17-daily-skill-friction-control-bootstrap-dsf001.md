...
- A dedicated pinned-action `macos-15` CI lane runs the complete host bootstrap suite plus native LaunchAgent plist validation so Darwin rename, fork, process-group, ACL, and launchd behavior is exercised on the deployment platform rather than inferred from Linux-only coverage.
