...
- The control repository is public so its pinned GitHub-hosted CI can run without private-repository billing; the separate case ledger remains private and no control-state or ledger data is committed here.
...
- Atomic replacement binds content and access policy independently: after closing the retained writer, the staged file must still contain the exact requested bytes and exact requested mode before no-replace or exchange publication. Managed Git exclusion, LaunchAgent reload receipt, canonical freshness stamp, weekly pair receipt, and LaunchAgent installation callers independently re-read the final pathname and bind the exact expected payload before commit. Observed pre-publication mutation leaves a new authority absent or the prior target unchanged, and transaction commit revalidates the published snapshot; this does not claim continuous exclusion of a same-UID change-and-restore between discrete reads.
...
- The LaunchAgent environment trampoline treats fixed `/bin/sh`, `/usr/bin/env`, and their OS loader stages as pre-execution trust roots. Its controlled variables are guaranteed only after `env -i` takes effect for the Python child; inherited tool controls are absent while macOS may add its own `__CF_USER_TEXT_ENCODING` platform metadata. Tests do not pretend user-space cleanup can repair a loader injection that aborts either trusted executable before that boundary.
...
- Delegated helper ABI validation preserves the main manifest's ordered `retired_repo_names` tuple. The exact current four-field `WorkspaceConfig` receives that bound tuple; the legacy three-field shape remains compatible only when the tuple is empty, and every other field sequence fails closed. The independent host manifest requires that tuple to remain empty so its synthetic control-only config cannot silently discard retired names.
...
- The macOS lane accepts CoreFoundation's decimal or hexadecimal spelling for numeric `__CF_USER_TEXT_ENCODING` fields while still requiring the Python child's exact managed environment after removing that OS-supplied metadata. It keeps the complete host and state suites under a 45-minute job-execution timeout. Across different heads, the only two public-runner executions that reached the full suite completed pytest in 14:44 and 29:03, with total job times of 15:03 and 29:24 respectively. The higher cap therefore leaves 15:36 above the observed total-job maximum while trading a longer runaway-termination bound for lower false-timeout risk; queue time is outside this bound, and two failing-run samples do not establish a stable latency distribution.
...
- Generated CI pins every third-party Action to a full commit SHA, disables persisted checkout credentials, and selects exact Node, Python, uv, Go, Rust, actionlint, ShellCheck, and shfmt versions. ShellCheck comes from an official versioned Linux x86_64 release archive under a bounded HTTPS download, exact byte ceiling, and exact SHA-256 check rather than a mutable or potentially stalled runner package mirror; the complete lint-tool installation step retains a separate ten-minute hard timeout. Change those pins only through the authoritative generator and its round-trip contract when a reviewed dependency update requires it.
- Generated Linux CI and the native macOS lane install exact uv-managed Python 3.12.10 under a runner-owned temporary directory. The shared exact pin prevents the generated and host workflows from drifting, while the production policy test independently verifies the runtime interpreter's actual ancestry and access policy.
