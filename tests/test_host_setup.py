...
def _configure_managed_mirror_hooks(repository: Path) -> None:
    hooks_path = repository / ".git" / "hooks"
    _append_local_config(repository, f"[core]\n\thooksPath = {hooks_path}")
    expected = hs._managed_mirror_guard_data(repository)
    for hook_name in hs.MANAGED_MIRROR_GUARD_HOOKS:
        hook = hooks_path / hook_name
        hook.write_bytes(expected)
        hook.chmod(0o755)
...
def test_managed_mirror_hook_contract_matches_independent_fixed_fixture() -> None:
    mirror = Path("/private/tmp/dsf managed mirror")
    expected_names = (
        "pre-commit",
        "prepare-commit-msg",
        "pre-merge-commit",
    ...
    expected_payload = b"""#!/bin/sh
# Generated by codex-workspace. Do not edit.
mirror_path='/private/tmp/dsf managed mirror'
...
"""
    ...
    assert hs.MANAGED_MIRROR_GUARD_HOOKS == expected_names
    assert hs._managed_mirror_guard_data(mirror) == expected_payload


def test_installed_workspace_helper_matches_independent_managed_hook_contract() -> None:
    config = hs.load_config(REPO_ROOT / "config" / "host-workspace.toml")
    helper = config.workspace_helper
    if not helper.is_file():
        pytest.skip(f"stable workspace helper is unavailable in this checkout: {helper}")
    namespace = runpy.run_path(str(helper))
    authority_names = namespace.get("MIRROR_GUARD_HOOKS")
    raw_authority_renderer = namespace.get("mirror_guard_hook")
    assert authority_names == (
        "pre-commit",
        "prepare-commit-msg",
        "pre-merge-commit",
        "applypatch-msg",
    ...
    assert callable(raw_authority_renderer)
    authority_renderer = cast(Callable[[Path], str], raw_authority_renderer)
    rendered = authority_renderer(Path("/private/tmp/dsf managed mirror"))
    assert (
        rendered.encode()
        == b"""#!/bin/sh
# Generated by codex-workspace. Do not edit.
...
def test_plan_accepts_current_workspace_and_managed_mirror_config_shapes(
    tmp_path: Path,
) -> None:
    fixture = _build_host(tmp_path)
    active = _active(fixture)
    _append_local_config(fixture.workspace, "[extensions]\n\tworktreeConfig = true")
    ...
    main_manifest = hs._load_main_manifest(active)
    mirrors = [
        manifest.repo_path(repo)
        for manifest in (control_manifest, main_manifest)
        for repo in manifest.repos
    ]
    for mirror in mirrors:
        _configure_managed_mirror_hooks(mirror)

    report = hs.plan_setup(active, fixture.home, FakeRunner(), no_launchctl=True)
    checks = {check["name"]: check for check in report["checks"]}

    assert checks["workspace-topology-replacements"]["status"] == "ready"
    for repo_name in (active.control_repo.name, "alpha", "beta"):
        assert checks[f"prefetch-{repo_name}-topology-replacements"]["status"] == "ready"
    assert not (fixture.workspace / ".git" / "config.worktree").exists()
    assert all(not (mirror / ".git" / "config.worktree").exists() for mirror in mirrors)
    assert not any(
        "selects executable or redirected Git behavior" in check["detail"]
        for check in report["checks"]
    )


def test_managed_mirror_hooks_path_is_bound_but_disabled_for_git_and_helper(
    tmp_path: Path,
) -> None:
    fixture = _build_host(tmp_path)
    active = _active(fixture)
    ...
    guard = hs._inspect_git_topology_replacements(
        mirror,
        allow_managed_hooks_path=True,
    )
    assert guard.hooks_binding is not None
    assert guard.managed_hook_snapshots is not None
    assert [name for name, _snapshot in guard.managed_hook_snapshots] == list(
        hs.MANAGED_MIRROR_GUARD_HOOKS
    )
    ...
    assert "default-branch mirror" in unguarded.stderr

    protected = hs._run_git(
        mirror,
        "-c",
        "user.name=Host Setup Test",
        "-c",
...
@pytest.mark.parametrize(
    "setting_kind",
    ["external", "relative", "missing-value", "duplicate", "subsection"],
)
def test_managed_mirror_hooks_path_profile_rejects_nonexact_shapes_before_git(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    setting_kind: str,
) -> None:
    ...
    expected = mirror / ".git" / "hooks"
    setting = {
        "external": "[core]\n\thooksPath = /tmp/external-hooks",
        "relative": "[core]\n\thooksPath = .git/hooks",
        "missing-value": "[core]\n\thooksPath",
        "duplicate": (f"[core]\n\thooksPath = {expected}\n\thooksPath = {expected}"),
        "subsection": f'[core "hostile"]\n\thooksPath = {expected}',
    }[setting_kind]
    _append_local_config(mirror, setting)
    started = False

    def forbidden_run(*_args: object, **_kwargs: object) -> subprocess.CompletedProcess[str]:
        nonlocal started
        started = True
        raise AssertionError("Git must not start after an invalid hooksPath is found")

    monkeypatch.setattr(hs.CommandRunner, "run", forbidden_run)
    with pytest.raises(hs.SetupError, match="hookspath"):
        hs._run_git(
            mirror,
            "status",
            "--porcelain=v1",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("extensions", "worktreeConfig = True"),
        ("extensions", "worktreeConfig = yes"),
        ('extensions "hostile"', "worktreeConfig = true"),
    ],
)
def test_workspace_worktree_config_profile_rejects_nonexact_values_before_git(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    section: str,
    setting: str,
    ...
    fixture = _build_host(tmp_path)
    _append_local_config(fixture.workspace, f"[{section}]\n\t{setting}")
    started = False

    def forbidden_run(*_args: object, **_kwargs: object) -> subprocess.CompletedProcess[str]:
        nonlocal started
        started = True
        raise AssertionError("Git must not start after invalid worktreeConfig is found")

    monkeypatch.setattr(hs.CommandRunner, "run", forbidden_run)
    with pytest.raises(hs.SetupError, match=r"extensions.*worktreeconfig"):
        hs._run_git(
            fixture.workspace,
            "status",
            "--porcelain=v1",
...
def test_git_config_profiles_do_not_cross_allow_settings(tmp_path: Path) -> None:
    fixture = _build_host(tmp_path)
    workspace_hooks = fixture.workspace / ".git" / "hooks"
    _append_local_config(fixture.workspace, f"[core]\n\thooksPath = {workspace_hooks}")
    with pytest.raises(hs.SetupError, match="core.hookspath"):
        hs._inspect_git_topology_replacements(
            fixture.workspace,
            allow_worktree_config=True,
        )

    mirror = fixture.config.cache_root / "repos" / "alpha"
    _append_local_config(mirror, "[extensions]\n\tworktreeConfig = true")
...
@pytest.mark.parametrize(
    ...
    [
        ("missing", "guard hook is missing"),
        ("content", "non-managed content"),
        ("mode", "non-managed access policy"),
        ("symlink", "hook leaf is a symlink"),
    ],
)
def test_managed_mirror_hook_leaf_must_be_exact_before_git(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
    expected_error: str,
    ...
    mirror = fixture.config.cache_root / "repos" / "alpha"
    _configure_managed_mirror_hooks(mirror)
    hook = mirror / ".git" / "hooks" / hs.MANAGED_MIRROR_GUARD_HOOKS[0]
    if mutation == "missing":
        hook.unlink()
        ...
        external.write_bytes(hs._managed_mirror_guard_data(mirror))
    ...
    def forbidden_run(*_args: object, **_kwargs: object) -> subprocess.CompletedProcess[str]:
        nonlocal started
        started = True
        raise AssertionError("Git must not start after a managed hook mismatch")
...
def test_local_config_symlink_replacement_is_rejected_before_git(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture = _build_host(tmp_path)
    mirror = fixture.config.cache_root / "repos" / "alpha"
    config = mirror / ".git" / "config"
    ...
    def forbidden_run(*_args: object, **_kwargs: object) -> subprocess.CompletedProcess[str]:
        nonlocal started
        started = True
        raise AssertionError("Git must not start through a replaced local config")

    monkeypatch.setattr(hs.CommandRunner, "run", forbidden_run)
    with pytest.raises(hs.SetupError, match="Git local config"):
        hs._run_git(mirror, "status", "--porcelain=v1")
    assert started is False


@pytest.mark.parametrize("entrypoint", ["git", "helper"])
def test_local_config_same_byte_replacement_fails_post_child_revalidation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    entrypoint: str,
) -> None:
    ...
    if entrypoint == "git":
        ...
        monkeypatch.setattr(hs.CommandRunner, "run", replace_during_git)
        with pytest.raises(hs.SetupError, match="Git local config changed"):
            hs._run_git(
                mirror,
                "status",
                "--porcelain=v1",
        ...
        with pytest.raises(hs.SetupError, match="Git local config changed"):
            hs._run_helper(
                ...
                manifest,
                ["status", "--repo", manifest.repos[0].name],
                FakeRunner(on_helper=lambda _args: replace_config()),
            )


@pytest.mark.parametrize("entrypoint", ["git", "helper"])
def test_managed_hook_same_byte_replacement_fails_post_child_revalidation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    entrypoint: str,
) -> None:
    ...
    def replace_hook() -> None:
        replacement = hook.with_name(f"{hook.name}.replacement")
    ...
    if entrypoint == "git":
        ...
        monkeypatch.setattr(hs.CommandRunner, "run", replace_during_git)
        with pytest.raises(hs.SetupError, match="managed mirror guard hooks changed"):
            hs._run_git(
                mirror,
                "status",
                "--porcelain=v1",
        ...
        with pytest.raises(hs.SetupError, match="managed mirror guard hooks changed"):
            hs._run_helper(
                ...
                manifest,
                ["status", "--repo", manifest.repos[0].name],
                FakeRunner(on_helper=lambda _args: replace_hook()),
            )


def test_managed_hooks_directory_replacement_fails_post_child_revalidation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture = _build_host(tmp_path)
    mirror = fixture.config.cache_root / "repos" / "alpha"
    _configure_managed_mirror_hooks(mirror)
    hooks = mirror / ".git" / "hooks"

    def replace_hooks_directory(
        _runner: hs.CommandRunner,
        argv: Sequence[str],
        **_kwargs: object,
    ) -> subprocess.CompletedProcess[str]:
        displaced = hooks.with_name("hooks.displaced")
        hooks.rename(displaced)
        hooks.mkdir(mode=0o755)
        expected = hs._managed_mirror_guard_data(mirror)
        for hook_name in hs.MANAGED_MIRROR_GUARD_HOOKS:
    ...
    monkeypatch.setattr(hs.CommandRunner, "run", replace_hooks_directory)
    with pytest.raises(hs.SetupError, match="hooks directory identity or access policy changed"):
        hs._run_git(
            mirror,
            "status",
            "--porcelain=v1",
...
def test_managed_hooks_directory_replacement_during_final_snapshot_reads_is_blocked(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture = _build_host(tmp_path)
    ...
    _configure_managed_mirror_hooks(mirror)
    hooks = mirror / ".git" / "hooks"
    real_snapshot = hs._managed_mirror_guard_snapshot
    snapshot_reads = 0

    def replace_live_hooks_after_final_read_starts(
        hooks_fd: int,
        hooks_path: Path,
        hook_name: str,
        expected_data: bytes,
        ...
        snapshot = real_snapshot(hooks_fd, hooks_path, hook_name, expected_data)
        snapshot_reads += 1
        if (
            snapshot_reads
            == len(
            ...
            displaced = hooks.with_name("hooks.displaced-during-snapshot")
            hooks.rename(displaced)
            hooks.mkdir(mode=0o755)
            fixed_payload = displaced.joinpath("pre-commit").read_bytes()
            for fixed_name in (
                "pre-commit",
                "prepare-commit-msg",
                "pre-merge-commit",
                ...
                replacement = hooks / fixed_name
                replacement.write_bytes(fixed_payload)
                replacement.chmod(0o755)
        return snapshot
    ...
    monkeypatch.setattr(
        ...
        replace_live_hooks_after_final_read_starts,
    )
    with pytest.raises(hs.SetupError, match="hooks directory was replaced"):
        hs._inspect_git_topology_replacements(
            mirror,
            allow_managed_hooks_path=True,
        )
    assert snapshot_reads == 10


def test_workspace_worktree_config_creation_fails_post_child_revalidation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fixture = _build_host(tmp_path)
    _append_local_config(fixture.workspace, "[extensions]\n\tworktreeConfig = true")
    worktree_config = fixture.workspace / ".git" / "config.worktree"
    ...
    with pytest.raises(hs.SetupError, match="worktree config regular file is present"):
        hs._run_git(
            fixture.workspace,
            "status",
            "--porcelain=v1",
