...
`apply --ensure` is the only bootstrap path that may invoke the workspace helper's `ensure` command. It is intended for a first, explicitly authorized installation. Before `ensure`, bootstrap validates every existing workspace, cache, locator, Git-info, and LaunchAgents ancestor that the helper or installer may touch. It then performs the initial control and main prefetch pair, which may safely fast-forward a clean-behind mirror, proves the resulting manifests and mirrors with strict helper status, installs host state, and runs `doctor`; a successful first bootstrap is immediately ready. Plain `apply` never clones. Scheduled wrappers invoke only the helper's `prefetch` command and fail closed when a mirror is missing or unsafe. The manifest-bound `host_setup.account_home` must match the operating account home. After the environment trampoline succeeds, managed LaunchAgents supply Python only the fixed system `PATH`, that bound `HOME`, `LANG`, `TMPDIR`, and an optional `SSH_AUTH_SOCK`; macOS may independently attach its `__CF_USER_TEXT_ENCODING` platform metadata. Use `plan` or `doctor` to diagnose drift before applying recovery.

The fixed `/bin/sh`, `/usr/bin/env`, and their OS loader stages are pre-execution trust roots. Only after `/usr/bin/env` successfully applies `-i` does the Python child receive the controlled variables above; inherited tool-control variables are absent even if the platform adds its own runtime metadata. This does not claim that user-space code can sanitize a hostile loader setting that prevents either trusted system executable from reaching that boundary.
